...
test("Vercel is configured for TanStack Start", () => {
  ...
  assert.equal(v.framework, "tanstack-start");
});
test("SSR client entry uses StartClient hydration", () =>
  assert.match(read("src/client.tsx"), /StartClient/));
test("Supabase configuration has one public source of truth", () => {
  const client = read("src/integrations/supabase/client.ts");
  assert.match(client, /config\/public/);
  assert.doesNotMatch(client, /VITE_SUPABASE_URL\s*\|\|\s*["']/);
});
test("Admin auth cannot grant access from local mock state", () => {
  const hook = read("src/hooks/useAdmin.ts");
  assert.doesNotMatch(hook, /localStorage|getItem\(|mock.*admin/i);
  assert.match(hook, /admin_users/);
});
test("Admin route boundary is isolated from the control-room implementation", () =>
  assert.match(read("src/routes/admin.lazy.tsx"), /components\/admin\/AdminControlRoom/));
...
test("Package manifest and lockfile root dependencies are compatible", () => {
  const pkg = JSON.parse(read("package.json"));
  const lock = JSON.parse(read("package-lock.json"));
  const root = lock.packages?.[""];
  assert.ok(root, "package-lock root package entry is missing");
  for (const [name, range] of Object.entries(pkg.dependencies))
    assert.equal(root.dependencies?.[name], range, `dependency ${name} differs`);
  for (const [name, range] of Object.entries(pkg.devDependencies)) {
    assert.equal(root.devDependencies?.[name], range, `devDependency ${name} differs`);
  }
