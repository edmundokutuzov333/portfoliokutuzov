...
test("AI assistant is not part of the critical shell",()=>{const file=read("src/components/DeferredAiAssistant.tsx");assert.match(file,/lazy\(/);assert.match(file,/requestIdleCallback/);assert.match(file,/timeout:2500/)})
...
test("Route cache avoids aggressive refetching",()=>{const router=read("src/router.tsx");assert.match(router,/staleTime:120_000/);assert.match(router,/gcTime:15\*60_000/);assert.match(router,/defaultPreloadStaleTime:30_000/)})
