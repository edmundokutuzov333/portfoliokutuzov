...
test("Heavy reel is deferred until the browser is idle", () => {
...
test("Route cache avoids aggressive refetching", () => {
  ...
  assert.match(router, /gcTime\s*:\s*15\*60_000/);
  assert.match(router, /defaultPreloadStaleTime\s*:\s*30_000/);
});
test("Motion/accessibility tokens include reduced-motion and focus treatment", () => {
