import { c as createServerRpc } from "./createServerRpc-C2RwQW1L.mjs";
import { c as createServerFn } from "./server-D2mK8el-.mjs";
import "../_libs/seroval.mjs";
import "../_libs/react.mjs";
import { f as object, l as literal, _ as _enum, d as string } from "../_libs/zod.mjs";
...
async function addToResendAudience(email, name) {
  ...
  try {
    const res = await fetch(`${RESEND_GATEWAY}/audiences/${audienceId}/contacts`, {
      ...
      body: JSON.stringify({
        email,
        first_name: name?.split(" ")[0] ?? void 0,
        last_name: name?.split(" ").slice(1).join(" ") || void 0,
        unsubscribed: false
      })
    });
...
const subscribeNewsletter = createServerFn({
  method: "POST"
}).inputValidator((input) => SubscribeInput.parse(input)).handler(subscribeNewsletter_createServerFn_handler, async ({
  data
}) => {
  const {
    supabaseAdmin
  } = await import("./index.server-BMj2q4-x.mjs");
  const {
    data: existing
  } = await supabaseAdmin.from("newsletter_subscribers").select("id, is_active").eq("email", data.email).maybeSingle();
  if (existing) {
