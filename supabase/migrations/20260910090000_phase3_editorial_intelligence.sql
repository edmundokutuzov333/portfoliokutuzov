...
-- RLS: public visitors may read only published editorial content. Writes remain server/admin controlled.
...
create policy "Published project sections are public" on public.project_sections
  for select using (
    is_published and exists (
      select 1 from public.projects p
      where p.id = project_sections.project_id and p.is_published = true
    )
  );

create policy "Published project media are public" on public.project_media
  for select using (
    is_published and exists (
      select 1 from public.projects p
...
create policy "Project metrics are public for published projects" on public.project_metrics
  for select using (exists (select 1 from public.projects p where p.id = project_metrics.project_id and p.is_published = true));

create policy "Project credits are public for published projects" on public.project_credits
  for select using (exists (select 1 from public.projects p where p.id = project_credits.project_id and p.is_published = true));

create policy "Project relations are public for published projects" on public.project_relations
  for select using (
