-- Phase 4 editorial permission refinement.
-- Invoice settings remain finance-owned even though they live in site_settings.
...
BEGIN
  ...
  END IF;
  IF COALESCE(array_length(p_draft_ids,1),0)=0 THEN RAISE EXCEPTION 'No drafts selected'; END IF;
...
CREATE OR REPLACE FUNCTION public.admin_restore_audit_state(
  p_entity_type text,p_entity_id text,p_snapshot jsonb,p_audit_id uuid
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_entity_type='site_settings' THEN
    INSERT INTO public.site_settings(key,value,updated_at) VALUES(p_entity_id,p_snapshot,now())
    ON CONFLICT(key) DO UPDATE SET value=EXCLUDED.value,updated_at=now();
  ELSIF p_entity_type='projects' THEN
    INSERT INTO public.projects SELECT * FROM jsonb_populate_record(NULL::public.projects,p_snapshot || jsonb_build_object('id',p_entity_id::uuid))
    ON CONFLICT(id) DO UPDATE SET
      title=EXCLUDED.title,subtitle=EXCLUDED.subtitle,category=EXCLUDED.category,year=EXCLUDED.year,
      ...
      collaborators=EXCLUDED.collaborators,tools_used=EXCLUDED.tools_used,deliverables=EXCLUDED.deliverables,video_url=EXCLUDED.video_url,video_provider=EXCLUDED.video_provider,
      client_id=EXCLUDED.client_id,updated_at=now();
  ELSIF p_entity_type='clients' THEN
    INSERT INTO public.clients SELECT * FROM jsonb_populate_record(NULL::public.clients,p_snapshot || jsonb_build_object('id',p_entity_id::uuid))
    ON CONFLICT(id) DO UPDATE SET
  ...
  ELSIF p_entity_type='services' THEN
    INSERT INTO public.services SELECT * FROM jsonb_populate_record(NULL::public.services,p_snapshot || jsonb_build_object('id',p_entity_id::uuid))
    ON CONFLICT(id) DO UPDATE SET
  ...
  ELSIF p_entity_type='stats' THEN
    INSERT INTO public.stats SELECT * FROM jsonb_populate_record(NULL::public.stats,p_snapshot || jsonb_build_object('id',p_entity_id::uuid))
    ON CONFLICT(id) DO UPDATE SET
      value=EXCLUDED.value,label=EXCLUDED.label,sort_order=EXCLUDED.sort_order,is_active=EXCLUDED.is_active,updated_at=now();
  ELSIF p_entity_type='about_method' THEN
    INSERT INTO public.about_method SELECT * FROM jsonb_populate_record(NULL::public.about_method,p_snapshot || jsonb_build_object('id',p_entity_id::uuid))
    ON CONFLICT(id) DO UPDATE SET
  ...
  RETURN jsonb_build_object('ok',true,'audit_id',p_audit_id,'entity_type',p_entity_type,'entity_id',p_entity_id);
EXCEPTION
