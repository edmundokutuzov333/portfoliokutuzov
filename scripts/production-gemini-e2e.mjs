...
async function requestJson(path, init = {}) {
  const response = await withTimeout(fetch(`${baseUrl}${path}`, { ...init, headers: { Accept: "application/json", ...(init.headers || {}) } }), path);
...
assert(Array.isArray(health.body?.capabilities), "Production /api/chat did not expose capabilities");

const opening = await requestJson("/api/chat", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({ action: "opening_message", sessionId: `prod-e2e-${Date.now()}`, context: { pathname: "/", userLanguage: "en" } }),
});
assert(opening.response.ok, `opening_message returned HTTP ${opening.response.status}`);
assert(typeof opening.body?.message === "string" && opening.body.message.trim().length > 0, "opening_message returned no assistant message");

const chat = await withTimeout(fetch(`${baseUrl}/api/chat`, {
  method: "POST",
  headers: { "Content-Type": "application/json", Accept: "text/event-stream" },
  body: JSON.stringify({
    sessionId: `prod-e2e-chat-${Date.now()}`,
    context: { pathname: "/portfolio", userLanguage: "en" },
    messages: [{ role: "user", text: "Which published project best represents your art direction work?" }],
  }),
}), "/api/chat SSE");

assert(chat.ok, `/api/chat SSE returned HTTP ${chat.status}`);
const sseText = await withTimeout(chat.text(), "/api/chat SSE body");
const events = sseText.split("\n\n").filter(Boolean).map((block) => block.replace(/^data:\s*/, "")).map((value) => {
  try { return JSON.parse(value); } catch { return null; }
}).filter(Boolean);
const hasText = events.some((event) => event.type === "chunk" && typeof event.text === "string" && event.text.trim().length > 0);
