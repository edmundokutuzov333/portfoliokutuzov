...
for (const file of fs.readdirSync(dir).filter((name) => name.endsWith(".json"))) {
  ...
  const failed = (category) => Object.values(lhr.categories?.[category]?.auditRefs ?? [])
    .map((ref) => audit(ref.id))
    .filter((item) => item && item.score !== null && item.score < 1)
    .map((item) => `${item.id}: ${item.title}`);
  console.log(`LH ${lhr.requestedUrl}: perf=${score("performance")} a11y=${score("accessibility")} best=${score("best-practices")} seo=${score("seo")}`);
  console.log(`  LCP=${audit("largest-contentful-paint")?.numericValue ?? "?"}ms element=${lcpNode}`);
  console.log(`  FCP=${audit("first-contentful-paint")?.numericValue ?? "?"}ms CLS=${audit("cumulative-layout-shift")?.numericValue ?? "?"} TBT=${audit("total-blocking-time")?.numericValue ?? "?"}ms`);
