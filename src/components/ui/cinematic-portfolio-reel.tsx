...
export function CinematicPortfolioReel() {
  ...
  const projects = React.useMemo(
    () => orderedProjects((data ?? []).filter((p) => Boolean(p.cover_url))),
    [data],
  );
...
function CarouselCard({
  ...
  const x = useTransform(offset, (v) => v * config.xMultiplier),
    y = useTransform(offset, (v) => (Math.abs(v) < 0.05 ? 0 : Math.abs(v) * config.yMultiplier)),
    rotate = useTransform(offset, (v) => (Math.abs(v) < 0.05 ? 0 : v * config.rotationMultiplier)),
    scale = useTransform(offset, (v) => 1 - Math.abs(v) * config.scaleReduction),
    opacity = useTransform(
      offset,
      [-total / 2, -total / 2 + 0.5, 0, total / 2 - 0.5, total / 2],
      [0, 1, 1, 1, 0],
    ),
    zIndex = useTransform(offset, (v) => Math.round(100 - Math.abs(v) * 10));
  return (
    <motion.div
    ...
    >
      <div className="relative w-full h-full bg-[#050505]">
        ...
        <div className="absolute bottom-6 left-6 right-6">
          <div className="mono text-[10px] uppercase tracking-[.2em] text-white/70 mb-2">
            {slide.category}
            {slide.year ? ` · ${slide.year}` : ""}
          </div>
          <h3 className="display text-2xl md:text-3xl text-white font-medium leading-[1.1] tracking-[-.02em]">
            {slide.title}
          </h3>
