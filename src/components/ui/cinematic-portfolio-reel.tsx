...
export function CinematicPortfolioReel() {
  ...
  const projects = React.useMemo(
    () => orderedProjects((data ?? []).filter((project) => Boolean(project.cover_url))),
    [data],
  );
  ...
  return (
    <section
      ...
      className="relative isolate z-0 w-full overflow-hidden bg-[var(--color-bg)] pt-28 pb-8 md:pt-32 md:pb-12"
    >
      <div className="relative z-[5] mb-10 flex w-full justify-center px-4">
        <motion.p
          id="portfolio-reel-title"
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
          className="mono text-[10px] uppercase tracking-[.3em] text-[var(--color-text-muted)]"
        >
...
function CarouselStacked({ projects }: { projects: DbProject[] }) {
  ...
  const count =
    width > 0 && width < 640
      ? Math.min(10, Math.max(6, projects.length))
      : Math.min(16, Math.max(8, projects.length * 2));
  const slides = React.useMemo(
    () => Array.from({ length: count }, (_, i) => projects[i % projects.length]),
    [count, projects],
  );
  ...
  const move = React.useCallback(
    (amount: number) => {
      setPaused(true);
      animate(progress, Math.round(progress.get()) + amount, {
        type: "spring",
        stiffness: 220,
        damping: 28,
        mass: 0.9,
  ...
  return (
    <div
      ref={ref}
      className="relative h-[50vh] w-full select-none overflow-hidden md:h-[60vh] lg:h-[70vh]"
    >
      <div className="relative flex h-full w-full items-center justify-center">
        <motion.button
          ...
          className="absolute inset-0 z-[20] cursor-grab border-0 bg-transparent p-0 active:cursor-grabbing touch-pan-y focus-visible:outline-2 focus-visible:outline-[var(--color-accent-hover)] focus-visible:outline-offset-[-4px]"
        />
      ...
      {!reduced && (
        <button
          ...
          className="absolute bottom-5 right-5 z-[25] inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/15 bg-black/50 text-white backdrop-blur-md transition hover:bg-black/70 focus-visible:outline-2 focus-visible:outline-[var(--color-accent-hover)]"
        >
...
function CarouselCard({
  ...
  return (
    <motion.div
      ...
      aria-hidden="true"
      className="absolute aspect-[4/5] h-[85%] w-auto overflow-hidden rounded-xl border border-[var(--color-border-subtle)] bg-black shadow-2xl"
    >
      <div className="relative h-full w-full bg-[#050505]">
        ...
        <div className="absolute bottom-6 left-6 right-6">
          <div className="mono mb-2 text-[10px] uppercase tracking-[.2em] text-white/70">
            {slide.category}
            {slide.year ? ` · ${slide.year}` : ""}
          </div>
          <h3 className="display text-2xl font-medium leading-[1.1] tracking-[-.02em] text-white md:text-3xl">
            {slide.title}
          </h3>
