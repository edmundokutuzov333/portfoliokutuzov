...
export function FeaturedWork() {
  ...
  return (
    <section
      className="relative px-4 md:px-8 py-32 bg-[var(--color-bg)]"
      ...
      <div className="max-w-[var(--width-wide)] mx-auto">
        ...
        >
          <h2
            ...
            className="display text-4xl sm:text-5xl lg:text-6xl leading-[1] tracking-[-.03em] text-[var(--color-text-primary)] max-w-xl"
          >
            {r("title", "Selected work")}
          </h2>
          <Link
            to="/portfolio"
            className="group flex min-h-11 items-center gap-2 text-[12px] mono tracking-[.15em] uppercase text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition"
          >
        ...
        <div className="mt-32 flex justify-center border-t border-[var(--color-border-subtle)] pt-16">
          <Link
            to="/portfolio"
            className="group flex min-h-14 items-center justify-center rounded-full bg-[var(--color-text-primary)] px-8 text-[13px] font-semibold text-[var(--color-bg)]"
          >
            View all projects
          </Link>
        </div>
...
function FeaturedCard({ project, index }: { project: DbProject; index: number }) {
  ...
  const image = (
    <img
      ...
      className="transition-transform duration-[1.5s] ease-[.16,1,.3,1] group-hover:scale-[1.02]"
    />
  );
  if (index === 0)
    return (
        ...
        >
          ...
          <div className="mt-6 md:mt-8 grid grid-cols-1 md:grid-cols-12 gap-4">
            <div className="md:col-span-8">
              <h3 className="display text-4xl md:text-5xl lg:text-6xl text-[var(--color-text-primary)]">
                {project.title}
              </h3>
            </div>
            <div className="md:col-span-4 md:text-right">
              <div className="mono text-[10px] uppercase tracking-[.2em] text-[var(--color-text-muted)]">
                {project.category}
  ...
  return (
    ...
    >
      ...
      <div className={clsx("md:col-span-5 flex flex-col", even ? "md:order-2" : "md:order-1")}>
        <Link
          to="/portfolio/$slug"
          params={{ slug: project.slug || project.id }}
          className="block focus-visible:outline-2 focus-visible:outline-[var(--color-accent-hover)]"
        >
          <div className="mono text-[10px] tracking-[.2em] uppercase text-[var(--color-text-muted)] mb-4">
            {project.category}
            {project.year ? ` · ${project.year}` : ""}
          </div>
          <h3 className="display text-3xl md:text-4xl lg:text-5xl text-[var(--color-text-primary)]">
            {project.title}
          </h3>
          {project.client_name && (
            <div className="mt-4 text-[15px] text-[var(--color-text-secondary)]">
              {project.client_name}
            </div>
          )}
          <div className="mt-8 flex items-center gap-3 text-[13px] font-medium">
            <span className="h-px w-8 bg-[var(--color-text-primary)]" />
            Explore project
          </div>
        </Link>
      </div>
