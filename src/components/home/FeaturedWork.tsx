...
export function FeaturedWork() {
  ...
  return (
    <section className="relative px-4 md:px-8 py-32 bg-[var(--color-bg)]" aria-labelledby="featured-work-title">
      <div className="max-w-[var(--width-wide)] mx-auto">
        ...
        >
          <div>
            <h2 id="featured-work-title" className="display text-4xl sm:text-5xl lg:text-6xl leading-[1] tracking-[-0.03em] text-[var(--color-text-primary)] max-w-xl">
          ...
          <Link
            ...
            className="group flex min-h-11 items-center gap-2 text-[12px] mono tracking-[0.15em] uppercase text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition"
          >
            Full Archive <ArrowUpRight size={14} className="opacity-60 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
          </Link>
        </motion.div>
        ...
        >
          <Link
            to="/portfolio"
            className="group flex min-h-14 items-center justify-center gap-3 rounded-full bg-[var(--color-text-primary)] px-8 text-[13px] font-semibold text-[var(--color-bg)] transition-all hover:bg-[var(--color-text-secondary)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent-hover)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--color-bg)]"
          >
            View all projects
          </Link>
        </motion.div>
...
function FeaturedCard({ project, index }: { project: DbProject; index: number }) {
    ...
    return (
      ...
      >
        <Link
          ...
          className="block focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent-hover)] focus-visible:ring-offset-4 focus-visible:ring-offset-[var(--color-bg)]"
        >
          ...
          <div className="mt-6 md:mt-8 grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
            ...
            <div className="md:col-span-4 md:text-right flex flex-col md:items-end justify-start pt-2">
              ...
              {project.client_name && <div className="text-[14px] font-medium text-[var(--color-text-secondary)]">Client: {project.client_name}</div>}
            </div>
          </div>
        </Link>
      </motion.div>
  ...
  return (
    ...
    >
      <div className={clsx("md:col-span-7 relative overflow-hidden bg-[var(--color-surface)]", isEven ? "md:order-1" : "md:order-2")}>
        <Link
          to="/portfolio/$slug"
          params={{ slug: project.slug || project.id }}
          aria-label={`View ${project.title}`}
          className="block focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent-hover)]"
        >
          {project.cover_url && (
            <img
              src={project.cover_url}
      ...
      <div className={clsx("md:col-span-5 flex flex-col", isEven ? "md:order-2" : "md:order-1")}>
        ...
        >
          ...
          {project.client_name && <div className="mt-4 text-[15px] font-medium text-[var(--color-text-secondary)]">{project.client_name}</div>}
          <div className="mt-8 flex items-center gap-3 text-[13px] font-medium text-[var(--color-text-primary)]">
            ...
            <span className="group-hover:text-[var(--color-accent-hover)] transition-colors">Explore project</span>
          </div>
        </Link>
      </div>
    </motion.div>
