...
export function Phase3OperationsOS({ onNavigate }: { onNavigate?: (section: string) => void }) {
  ...
  const tabsWithBadges = tabs.map((item) => ({
    ...item,
    badge: item.id === "leads" ? overviewQuery.data?.leads?.new ?? 0 : item.id === "bookings" ? overviewQuery.data?.bookings?.requested ?? 0 : 0,
  }));
...
function FinanceWorkspace() {
  return <div className="space-y-6"><SectionHeader kicker="Operations / Finance" title="Finance." description="Existing invoice workspace plus the new payment ledger and CRM linkage."/><InvoiceWorkspace/></div>;
}
