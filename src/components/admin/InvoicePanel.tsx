  ...
  return (
    <div className="mt-5">
      <div className="mono text-[10px] tracking-[0.2em] text-slate-500 mb-2 flex items-center gap-2 flex-wrap">
        ...
        <span
          className={`px-2 py-0.5 rounded border text-[10px] ${STATUS_TONE[status] ?? STATUS_TONE.draft}`}
        >
          {status.toUpperCase()}
        </span>
                  ...
                  <div key={idx} className="border border-white/[0.06] rounded p-3 bg-white/[0.01]">
                    <div className="flex gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        <IconBtn onClick={() => move(idx, -1)} title="Move up">
                          <ArrowUp size={12} />
                        </IconBtn>
                        <IconBtn onClick={() => move(idx, 1)} title="Move down">
                          <ArrowDown size={12} />
                        </IconBtn>
                        <IconBtn
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-2 mt-2 items-end">
                      ...
                      <Field label="Rate">
                        <input
                          ...
                          value={it.unit_price}
                          onChange={(e) =>
                            patchItem(idx, { unit_price: Number(e.target.value) || 0 })
                          }
                          className={inputCls}
                        />
                      </Field>
                      <Field label="Disc %">
                        <input
                          ...
                          onChange={(e) =>
                            patchItem(idx, { discount_pct: Number(e.target.value) || 0 })
                          }
                          className={inputCls}
                        />
                      </Field>
              ...
              {STATUS_ORDER.map((s) => (
                <button
                  ...
                  className={`px-2 py-1 rounded border text-[10px] transition-colors ${
                    s === status
                      ? STATUS_TONE[s]
                      : "border-white/10 text-slate-500 hover:text-slate-200 hover:border-white/25"
                  }`}
                >
                  {s}
                </button>
                  ...
                  {events.data.map((e) => (
                    <li key={e.id} className="flex items-start justify-between gap-3 text-[11.5px]">
                      <span className="text-slate-300">
                        {e.event_type}
                        {e.recipients?.length ? (
                          <span className="text-slate-500"> → {e.recipients.join(", ")}</span>
                        ) : null}
                      </span>
        ...
        >
          ...
          >
            ...
            <div className="px-4 py-3 border-t border-white/[0.06] flex justify-end gap-2">
              <Btn onClick={() => setPreviewOpen(false)} tone="ghost">
                Close
              </Btn>
              <Btn
                onClick={() => doSend("invoice")}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="mono text-[9px] tracking-[0.18em] text-slate-600 block mb-1">
        {label.toUpperCase()}
      </span>
      {children}
    </label>
