...
function StudioCanvas({ design, selectedId, onSelect, onChange, onInteractionStart, onInteractionEnd }: {
  design: StudioDesignDocument;
  selectedId: string | null;
  onSelect: (id: string | null) => void;
  onChange: (design: StudioDesignDocument) => void;
  onInteractionStart: (interaction: Interaction) => void;
  ...
  const handlePointerMove = (event: PointerEvent<SVGSVGElement>) => {
    ...
    if (interaction.mode === "drag") {
      onChange(updateElementPosition(current, interaction.id, p.x - interaction.startX, p.y - interaction.startY));
    } else {
      onChange(updateElementSize(current, interaction.id, interaction.startW + p.x - interaction.startX, interaction.startH + p.y - interaction.startY));
    }
  };
...
export function BusinessCardEditor() {
  ...
  const commit = (next: StudioDesignDocument) => {
  ...
  const direct = (next: StudioDesignDocument) => { designRef.current = next; setDesign(next); };
  const undo = () => { const previous = past.at(-1); if (!previous) return; setPast((items) => items.slice(0,-1)); setFuture((items) => [clone(designRef.current), ...items]); direct(previous); };
  const redo = () => { const next = future[0]; if (!next) return; setFuture((items) => items.slice(1)); setPast((items) => [...items, clone(designRef.current)]); direct(next); };
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      ...
      if (event.key === "Delete" || event.key === "Backspace") {
        if (selectedId && selectedId !== "accent") { event.preventDefault(); commit({ ...designRef.current, elements: designRef.current.elements.filter((item) => item.id !== selectedId) }); setSelectedId(null); }
        return;
      }
  ...
  function fieldChange(key: FieldKey, value: string) { setValues((v) => ({ ...v, [key]: value })); commit(updateTextElement(designRef.current, key, value)); }
  function changeStyle(next: StudioStyle) { setStyle(next); commit(createStudioDesign(next, values)); setSelectedId(null); }
  function align(value: "left"|"center"|"right"|"top"|"middle"|"bottom") { if (selectedId) commit(alignElement(designRef.current, selectedId, value)); }
  function layer(direction: "up"|"down") { if (selectedId) commit(reorderElement(designRef.current, selectedId, direction)); }
  ...
  async function save() {
    setSaveState("saving");
    const record = saveDraftLocally({ sessionId, ...values, design: designRef.current });
    try { if (record) await saveStudioCard({ sessionId, ...values, design: designRef.current, id: record.id }); } catch { /* local draft keeps the editor usable when the remote DB is unavailable */ }
    setSaveState("saved");
