...
function withMagnificBackground(design: StudioDesignDocument, dataUrl: string): StudioDesignDocument {
  ...
  return {
    ...design,
    elements: [backgroundElement, ...design.elements.filter((element) => element.id !== "magnific-background")],
  };
}
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-[360px_minmax(0,1fr)]">
          <section className="rounded-[26px] border border-white/10 bg-white/[.025] p-5">
            ...
            <label className="block">
              <span className="mono mb-2 block text-[9px] uppercase tracking-[.18em] text-white/35">{pt ? "Prompt do fundo" : "Background prompt"}</span>
              <textarea value={prompt} onChange={(event) => setPrompt(event.target.value.slice(0, 1200))} rows={7} placeholder={pt ? "Ex.: textura arquitectónica escura, luz lateral suave, profundidade cinematográfica, espaço negativo para informação de contacto" : "E.g. dark architectural texture, soft side light, cinematic depth, negative space for contact information"} className="w-full resize-none rounded-2xl border border-white/10 bg-black/20 px-3.5 py-3 text-sm text-white outline-none placeholder:text-white/20 focus:border-[var(--color-accent-base)]" />
            </label>
            <label className="mt-4 block">
              <span className="mono mb-2 block text-[9px] uppercase tracking-[.18em] text-white/35">{pt ? "Elementos a evitar" : "Avoid"}</span>
              <textarea value={negativePrompt} onChange={(event) => setNegativePrompt(event.target.value.slice(0, 800))} rows={4} className="w-full resize-none rounded-2xl border border-white/10 bg-black/20 px-3.5 py-3 text-xs leading-relaxed text-white/70 outline-none" />
            </label>
            <button type="button" onClick={generate} disabled={busy || prompt.trim().length < 3} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[var(--color-accent-base)] px-4 py-3 text-sm font-semibold text-black disabled:opacity-40"><ImagePlus size={17}/>{busy ? (pt ? "A gerar fundo…" : "Generating background…") : (pt ? "Gerar com Magnific" : "Generate with Magnific")}</button>
          ...
          <section className="rounded-[26px] border border-white/10 bg-white/[.025] p-4 sm:p-6">
            <div className="mb-4 flex items-center justify-between gap-4"><div><p className="mono text-[9px] uppercase tracking-[.2em] text-white/35">Preview</p><p className="mt-1 text-xs text-white/35">{result ? `${result.width} × ${result.height}px · ${result.model}` : (pt ? "O resultado aparece aqui." : "The generated result appears here.")}</p></div>{result && <button type="button" onClick={generate} disabled={busy || prompt.trim().length < 3} className="inline-flex items-center gap-2 rounded-full border border-white/10 px-3 py-2 text-xs text-white/65"><RefreshCw size={14}/>{pt ? "Regenerar" : "Regenerate"}</button>}</div>
            <div className="overflow-hidden rounded-[22px] border border-white/10 bg-[#0b1118]">
              {result ? <img src={result.dataUrl} alt={pt ? "Fundo gerado pelo Magnific" : "Magnific generated background"} className="block aspect-[1.8] h-auto w-full object-cover" /> : <div className="grid aspect-[1.8] place-items-center px-8 text-center text-sm text-white/25">{pt ? "Descreva o ambiente visual e gere o fundo." : "Describe the visual environment and generate the background."}</div>}
            </div>
            {result && <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-white/35">{pt ? "O fundo será colocado atrás dos elementos do cartão no documento local." : "The background will be placed behind the card elements in the local design document."}</p><button type="button" onClick={apply} className="rounded-full bg-white px-4 py-2 text-xs font-semibold text-black">{applied ? (pt ? "Aplicado" : "Applied") : (pt ? "Usar no cartão" : "Use in card")}</button></div>}
          </section>
        </div>
      </div>
    </div>
