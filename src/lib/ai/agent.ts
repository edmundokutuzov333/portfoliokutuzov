...
export async function processChatStream(
  ...
  const lastUserMessage =
    [...safeMessages].reverse().find((message) => message.role === "user")?.text || "";
  const key = cacheKey(lastUserMessage, context);
  const cached = responseCache.get(key);
  if (
    cached &&
    cached.expiresAt > Date.now() &&
    !safeMessages.some((message) => message.role === "assistant" && message.text === cached.text)
  ) {
  ...
  const knowledgeText = formatKnowledgeForModel(
    await getPortfolioKnowledgeSnapshot(lastUserMessage, context),
  );
  ...
  try {
      ...
      async (ai, modelName) => {
        ...
        while (iterations++ < 3) {
          ...
          for await (const chunk of responseStream) {
            ...
            if (chunk.text && toolCalls.length === 0) {
          ...
          for (const call of toolCalls) {
            ...
            if (call.name === "getProject" && toolResult.project)
              emit({
                type: "project_detail",
                project: toolResult.project as Record<string, unknown>,
              });
            if (call.name === "navigateAction") {
    ...
    emit({
      type: "error",
      error: {
        code: (error as { code?: string })?.code || "AI_REQUEST_FAILED",
