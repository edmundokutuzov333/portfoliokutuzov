...
export async function runAiKnowledgeReindex(lang: "en" | "pt-PT" = "en") {
  if (process.env.AI_RAG_ENABLED !== "true") return { ok: true, lang, indexed: 0, faq: 0, skipped: true as const };
  const permissionlessDb = supabaseAdmin as any;
  ...
  const { data: projects, error: projectError } = await permissionlessDb
    .from("projects")
  ...
  for (const project of projects ?? []) {
    ...
    for (const [index, content] of chunks(text).entries()) {
      sources.push({ sourceTable: "case_studies", sourceId: project.id, title: project.title || "Project", url: "/portfolio/" + project.slug, sourceKey: "project:" + project.id + ":" + index, content });
    }
  }

  const { data: services, error: serviceError } = await permissionlessDb
    .from("services").select("id,title,description,is_active").eq("is_active", true).order("sort_order");
  ...
  const { data: credentials, error: credentialsError } = await permissionlessDb.from("site_settings").select("id,value").eq("key", "credentials").maybeSingle();
  if (credentialsError) throw new Error(credentialsError.message);
  if (credentials) {
    for (const [index, content] of chunks("Credentials JSON: " + JSON.stringify(credentials.value)).entries()) {
      sources.push({ sourceTable: "credentials", sourceId: credentials.id, title: "Credentials", url: "/credentials", sourceKey: "credentials:" + credentials.id + ":" + index, content });
    }
  }

  for (const source of sources) {
    const vector = await embed(source.content);
    await permissionlessDb.from("knowledge_chunks").upsert({
      source_table: source.sourceTable, source_id: source.sourceId, source_key: source.sourceKey,
      source_title: source.title, source_url: source.url, lang, content: source.content,
      content_hash: hash(source.content), embedding: "[" + vector.join(",") + "]",
      is_published: true, updated_at: new Date().toISOString(),
    }, { onConflict: "source_key,lang,content_hash" });
  }
  return { ok: true, lang, indexed: sources.length, faq: 0, skipped: false as const };
}

export const reindexAiKnowledge = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((input: unknown) => ({
    lang: input && typeof input === "object" && input !== null && "lang" in input && (input as { lang?: unknown }).lang === "pt-PT" ? "pt-PT" : "en",
  } as const))
  .handler(async ({ data, context }) => {
    ...
    return runAiKnowledgeReindex(data.lang);
  });
