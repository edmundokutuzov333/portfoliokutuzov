...
export interface PortfolioKnowledgeSnapshot {
  ...
  platform: { routes: string[]; capabilities: string[]; stack: string[]; safeguards: string[] };
}
...
function normalize(value: string): string { return value.toLocaleLowerCase("en").normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim(); }
function tokens(value: string): string[] { return normalize(value).split(/[^a-z0-9+]+/i).filter((token) => token.length > 1); }
function scoreProject(project: NormalizedProject, query: string): number {
  ...
  let score = 0;
  for (const [field, weight] of fields) { const haystack = normalize(field); for (const token of qTokens) score += haystack === token ? weight * 2 : haystack.includes(token) ? weight : 0; }
  return score;
}
...
export async function getPortfolioKnowledgeSnapshot(query: string, context: ChatContext): Promise<PortfolioKnowledgeSnapshot> {
  ...
  const matchingProjects = projects.map((project) => ({ project, score: scoreProject(project, query) })).filter(({ score }) => score > 0).sort((a, b) => b.score - a.score || a.project.title.localeCompare(b.project.title)).slice(0, 8).map(({ project }) => asKnowledgeProject(project));
  ...
  return { source: "portfolio-records", generatedAt: new Date().toISOString(), visitorContext: { pathname: context.pathname, projectSlug: context.projectSlug, projectTitle: context.projectTitle, selectedCategory: context.selectedCategory }, matchingProjects, allProjects, currentProject: currentProject ? asKnowledgeProject(currentProject) : undefined, taxonomy: { categories, disciplines, years, clients, tags }, platform: PLATFORM_KNOWLEDGE };
}
...
export function formatKnowledgeForModel(snapshot: PortfolioKnowledgeSnapshot, options: { includeAllProjects?: boolean } = {}): string {
  return JSON.stringify({
    ...
    site: { name: SITE_INFO.name, title: SITE_INFO.title, location: SITE_INFO.location, bio: SITE_INFO.bio, availability: SITE_INFO.availability, metrics: { years: SITE_INFO.yearsOfExperience, projects: SITE_INFO.projectsDelivered, brands: SITE_INFO.brandsCollaborated, continents: SITE_INFO.continentsActive } },
