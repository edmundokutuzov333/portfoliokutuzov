...
export interface PortfolioKnowledgeSnapshot {
  ...
  visitorContext: Pick<
    ChatContext,
    "pathname" | "projectSlug" | "projectTitle" | "selectedCategory"
  >;
...
const PLATFORM_KNOWLEDGE = {
  ...
  capabilities: [
    ...
    "case-study discovery and related work",
    "AI text streaming",
    "real-time AI voice conversation",
    "live speech transcription",
    "text-to-speech playback",
    "portfolio search and command palette",
    "contact project briefing",
    "booking request",
    "client logo system",
  ...
  safeguards: [
    ...
    "ephemeral Live API voice tokens",
    "origin allowlist",
    "rate limiting",
    "request size limits",
    "published-data grounding",
  ],
} as const;
...
function scoreProject(project: NormalizedProject, query: string): number {
  ...
  for (const [field, weight] of fields) {
    ...
    for (const token of qTokens)
      score += haystack === token ? weight * 2 : haystack.includes(token) ? weight : 0;
  }
  return score;
}
...
export async function getPortfolioKnowledgeSnapshot(
  ...
  const disciplines = [
    ...new Set(projects.map((project) => project.discipline).filter(Boolean)),
  ].sort();
...
export function formatKnowledgeForModel(
  snapshot: PortfolioKnowledgeSnapshot,
  options: { includeAllProjects?: boolean } = {},
): string {
  return JSON.stringify(
    {
      mandate:
        "This snapshot is the source of truth for public portfolio facts. Never invent facts that are absent.",
      site: {
        name: SITE_INFO.name,
      ...
      allProjects: options.includeAllProjects
        ? snapshot.allProjects.map(asCompactProject)
        : undefined,
