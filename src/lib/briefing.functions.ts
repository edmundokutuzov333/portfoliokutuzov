...
async function send(from: string, to: string[], subject: string, html: string) {
  ...
  try {
    ...
    // Email delivery is best-effort after the submission is persisted.
  }
}
...
export const sendBriefingEmails = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (brief.confirmation_sent_at) return { ok: true, skipped: "already-sent" as const };
    ...
    const clientHtml = shell(`
      ...
      <p style="font-size:15px;line-height:1.7;color:#cbd5e1;margin:0 0 18px">Your briefing has landed. I read every submission personally and will get back to you within 48 hours with next steps.</p>
      ...
      <p style="font-size:13px;line-height:1.7;color:#94a3b8;margin:16px 0 0">In the meantime, feel free to reply to this email if anything else comes to mind.</p>
    `);
    await send(FROM, [brief.email], "Brief received - Edmundo Kutuzov", clientHtml);

    const refs = refLinks.map((link) => esc(link.url)).join("<br>");
    const atts = attachments.map((attachment) => `<a href="${esc(attachment.url)}" style="color:#7dd3fc">${esc(attachment.name)}</a>`).join("<br>");
    const adminHtml = shell(`
      ...
      <div style="margin:20px 0"><p style="font-family:monospace;letter-spacing:.18em;color:#7dd3fc;font-size:11px;margin:0 0 8px">MESSAGE</p><p style="font-size:14px;line-height:1.7;color:#e2e8f0;white-space:pre-wrap;margin:0">${esc(brief.message)}</p></div>
