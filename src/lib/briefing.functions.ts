...
import { z } from "zod";
import { scoreLead } from "@/lib/lead-intelligence";
...
export const sendBriefingEmails = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const lead = scoreLead({
      ...
      exactAmount: brief.exact_amount,
      companyName: brief.company_name,
      country: brief.country,
      preferredContactMethod: brief.preferred_contact_method,
    });
    ...
    const adminHtml = shell(`
      ...
      <div style="border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:18px 20px">
        <p style="margin:4px 0;font-size:14px"><b>Lead score:</b> ${lead.score}/100</p>
      ...
      <p style="font-size:12px;color:#94a3b8;margin:16px 0">Signals: ${esc(lead.signals.join(", ") || "none")}</p>
