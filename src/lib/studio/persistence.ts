...
function read(key: string) {
  return sessionStorageSafe()?.getItem(key) || localStorageSafe()?.getItem(key) || undefined;
}
...
export function getStudioDraftMeta() {
  return {
    draftToken: read(TOKEN_KEY),
    id: read(CARD_ID_KEY),
    revision: Number(read(REVISION_KEY) || 0) || undefined,
  };
}

function remember(card: StudioSavedDraft) {
  if (!card.draftToken || !card.id || !card.revision) return;
  write(TOKEN_KEY, card.draftToken);
  write(CARD_ID_KEY, card.id);
  write(REVISION_KEY, String(card.revision));
}
