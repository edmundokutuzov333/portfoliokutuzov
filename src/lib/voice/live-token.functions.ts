...
export const createLiveVoiceToken = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const systemInstruction = [
      ...
      `Speak in ${language}. Use natural local phrasing, not literal translation.` ,
      "Be extremely fast, concise, intelligent, and useful. Expand only when the question requires detail.",
      ...
      "When a requested fact is absent, say that it is not present in the portfolio records.",
      ...
      `PORTFOLIO KNOWLEDGE:\n${formatKnowledgeForModel(knowledge)}`,
    ].join("\n\n");
