...
export class LiveVoiceSession {
  ...
  async start() {
    ...
    try {
      ...
      const token = (await createLiveVoiceToken({
        data: { context: this.context, locale: this.events.locale === "pt-PT" ? "pt-PT" : "en" },
      })) as TokenResult;
      ...
      this.setState("error");
      this.events.onError?.(
        error instanceof Error ? error.message : "Unable to start the real-time voice session.",
      );
    }
  }
  ...
  private handleMessage(raw: string) {
    ...
    if (message.error?.message) {
      this.setState("error");
      this.events.onError?.(message.error.message);
      return;
    }
    ...
    for (const part of content.modelTurn?.parts ?? []) {
      ...
      if (part.text) {
        ...
        this.events.onTranscriptChunk?.(part.text, false, "assistant");
      }
    }
    ...
    if (content.turnComplete) {
      ...
      if (assistantText) {
        ...
        this.events.onTranscriptChunk?.(assistantText, true, "assistant");
      }
      this.userTranscript = "";
      this.assistantTranscript = "";
      this.setState("listening");
