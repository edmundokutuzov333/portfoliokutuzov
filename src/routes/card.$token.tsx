...
function PublicDigitalCard() {
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("studio_cards").select("name,role,company,email,phone,website,design,published_at,public_enabled").eq("share_token", token).eq("status", "published").eq("public_enabled", true).maybeSingle();
  ...
  async function share() {
    ...
    else await safeClipboardWrite(url);
  }

  function downloadVCard() {
    const blob = new Blob([vcard], { type: "text/vcard;charset=utf-8" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `${(card.name || card.company || "contact").replace(/[^a-z0-9-_]+/gi, "-").toLowerCase()}.vcf`; a.click(); setTimeout(() => URL.revokeObjectURL(a.href), 1000);
  }
  ...
  return <main className="min-h-screen bg-[#02050c] px-5 py-12 sm:py-20"><div className="mx-auto max-w-[920px]">
    ...
    <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><a href={`mailto:${card.email}`} className="rounded-2xl border border-white/10 bg-white/[.025] p-4 text-white"><Mail size={16}/><span className="mt-3 block text-xs">{pt ? "Email" : "Email"}</span></a><a href={`tel:${card.phone}`} className="rounded-2xl border border-white/10 bg-white/[.025] p-4 text-white"><Phone size={16}/><span className="mt-3 block text-xs">{pt ? "Telefonar" : "Call"}</span></a><a href={website || "#"} target="_blank" rel="noopener noreferrer" className="rounded-2xl border border-white/10 bg-white/[.025] p-4 text-white"><Globe2 size={16}/><span className="mt-3 block text-xs">Website</span></a><button type="button" onClick={downloadVCard} className="rounded-2xl border border-white/10 bg-white/[.025] p-4 text-left text-white"><Download size={16}/><span className="mt-3 block text-xs">{pt ? "Guardar contacto" : "Save contact"}</span></button></div>
