...
const PUBLIC_EVENTS = new Set([
  ...
  "digital_card_view", "digital_card_share", "digital_card_save_contact", "digital_card_email_click", "digital_card_phone_click", "digital_card_website_click",
] as const);
const WINDOW_MS = 5 * 60 * 1000; const MAX_EVENTS = 80; const MAX_BODY_BYTES = 16 * 1024;
const rateLimits = new Map<string, { count: number; resetAt: number }>();
type PublicEvent = typeof PUBLIC_EVENTS extends Set<infer T> ? T : never;
...
export const Route = createFileRoute("/api/studio/admin")({ server: { handlers: {
  ...
  POST: async ({ request }) => {
    ...
    const shareToken = typeof event.shareToken === "string" ? event.shareToken.trim().slice(0, 128) : null; const sessionId = typeof event.sessionId === "string" ? event.sessionId.trim().slice(0, 128) : null;
    let cardId: string | null = null;
    if (shareToken) { const db = supabaseAdmin as any; const { data: card } = await db.from("studio_cards").select("id").eq("share_token", shareToken).eq("status", "published").eq("public_enabled", true).maybeSingle(); if (!card) return json(request, requestId, 404, { error: "CARD_NOT_FOUND" }); cardId = String(card.id); }
    await trackStudioEvent({ eventName: eventName as any, sessionId, cardId, shareToken, provider: typeof event.provider === "string" ? event.provider : null, exportFormat: typeof event.exportFormat === "string" ? event.exportFormat : null, durationMs: typeof event.durationMs === "number" ? event.durationMs : null, metadata: typeof event.metadata === "object" && event.metadata ? event.metadata as Record<string, unknown> : undefined });
    return json(request, requestId, 202, { ok: true });
  },
  GET: async ({ request }) => { const requestId = getRequestId(request); const auth = await authenticatedAdmin(request); if (!auth.ok) return json(request, requestId, auth.status, { error: auth.message }); const url = new URL(request.url); const daysRaw = Number(url.searchParams.get("days") || 30); const days = Number.isFinite(daysRaw) ? Math.max(1, Math.min(Math.round(daysRaw), 365)) : 30; const until = new Date(); const since = new Date(until.getTime() - days * 86_400_000); const { data, error } = await auth.client.rpc("studio_admin_dashboard", { p_since: since.toISOString(), p_until: until.toISOString() }); if (error) return json(request, requestId, 502, { error: "ANALYTICS_QUERY_FAILED" }); return json(request, requestId, 200, data ?? {}); },
} } });
