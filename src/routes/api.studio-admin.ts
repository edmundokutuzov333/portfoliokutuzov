...
function checkRateLimit(key: string) {
  ...
  if (current.count >= MAX_EVENTS) return { allowed: false, retryAfter: Math.max(1, Math.ceil((current.resetAt - now) / 1000)) };
  current.count += 1;
  return { allowed: true, retryAfter: 0 };
}

function json(request: Request, requestId: string, status: number, payload: unknown, extra: Record<string, string> = {}) {
  return new Response(JSON.stringify(payload), { status, headers: headers(request, requestId, { "Content-Type": "application/json", "Cache-Control": "no-store", ...extra }) });
}
...
async function authenticatedAdmin(request: Request) {
  ...
  const url = process.env.SUPABASE_URL || publicConfig.supabase.url;
  const key = process.env.SUPABASE_PUBLISHABLE_KEY || publicConfig.supabase.publishableKey;
  ...
  if (claims.error || !claims.data?.claims?.sub) return { ok: false as const, status: 401, message: "Invalid token" };
  const admin = await client.rpc("is_admin");
  if (admin.error || !admin.data) return { ok: false as const, status: 403, message: "Admin access required" };
  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        let body: any;
        try { body = await request.json(); } catch { return json(request, requestId, 400, { error: "INVALID_JSON" }); }
        ...
        const shareToken = typeof body?.shareToken === "string" ? body.shareToken.trim().slice(0, 128) : null;
        const sessionId = typeof body?.sessionId === "string" ? body.sessionId.trim().slice(0, 128) : null;
        let cardId: string | null = typeof body?.cardId === "string" ? body.cardId.trim() : null;
        if (shareToken) {
        ...
        await trackStudioEvent({ eventName: eventName as PublicEvent, sessionId, cardId, shareToken, provider: typeof body?.provider === "string" ? body.provider : null, exportFormat: typeof body?.exportFormat === "string" ? body.exportFormat : null, durationMs: typeof body?.durationMs === "number" ? body.durationMs : null, metadata: typeof body?.metadata === "object" && body.metadata ? body.metadata : undefined });
        return json(request, requestId, 202, { ok: true });
      },
      GET: async ({ request }) => {
        const requestId = getRequestId(request);
        const auth = await authenticatedAdmin(request);
        if (!auth.ok) return json(request, requestId, auth.status, { error: auth.message });
        const url = new URL(request.url);
        const daysRaw = Number(url.searchParams.get("days") || 30);
        const days = Number.isFinite(daysRaw) ? Math.max(1, Math.min(Math.round(daysRaw), 365)) : 30;
