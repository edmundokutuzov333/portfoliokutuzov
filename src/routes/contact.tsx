...
import {
  ...
  Paperclip, X, Loader2, AlertCircle, ArrowLeft, ArrowRight, CalendarDays,
  User, Briefcase, Wallet, ImagePlus,
} from "lucide-react";
import { useSiteSettings } from "@/hooks/useSiteData";
import {
  readSetting, SITE_EMAIL, SITE_PHONE,
} from "@/lib/cms";
...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Smart project briefing for new collaborations with Edmundo Kutuzov, art director in Maputo." },
...
function ContactPage() {
  ...
  useEffect(() => { trackEvent({ action: "view", element: "contact" }); }, []);
  ...
  const onFiles = async (list: FileList | null) => {
    ...
    if (files.length + incoming.length > MAX_FILES) { toast.error(`Max ${MAX_FILES} files`); return; }
    setUploading(true);
    try {
      const uploads: BriefingAttachment[] = [];
      for (const file of incoming) {
        if (file.size > MAX_SIZE) { toast.error(`${file.name} is over 8 MB`); continue; }
        ...
        uploads.push({ url: data.publicUrl, name: file.name, size: file.size, width: dims.w, height: dims.h });
      }
  ...
  const next = () => {
    if (!validateStep(step)) { toast.error("Please complete the required fields"); return; }
        ...
        <div className="mt-8 grid md:grid-cols-12 gap-10">
          ...
          <div className="md:col-span-5 space-y-8">
            ...
            <div className="space-y-3">
              <a href={`mailto:${email}`} onClick={() => trackEvent({ action: "click", element: "email" })} className="flex items-center gap-3 group">
              ...
              <a href={`tel:${String(phone).replace(/\s/g, "")}`} onClick={() => trackEvent({ action: "click", element: "phone" })} className="flex items-center gap-3 group">
                <span className="h-10 w-10 rounded-full border border-white/10 grid place-items-center group-hover:border-sky-300/50 transition">
                  <Phone size={15} />
                </span>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                type="button"
                onClick={() => { trackEvent({ action: "click", element: "booking-open" }); setBookingOpen(true); }}
            ...
            {done ? (
              <div className="py-14 text-center">
                ...
                <p className="text-sm text-slate-400 mt-2 max-w-sm mx-auto">
                  Thanks {fullName.split(" ")[0] || "for reaching out"}. I'll get back to you at <span className="text-sky-200">{emailVal}</span> within 48 hours.
                </p>
                ...
                <button
                  onClick={() => { setDone(false); setStep(1); setFullName(""); setEmailVal(""); setMessage(""); setFiles([]); }}
                      ...
                      <li key={s.id} className="flex-1 flex items-center gap-2 min-w-0">
                        <button type="button" onClick={() => setStep(s.id)}
                          className={`h-7 w-7 shrink-0 rounded-full grid place-items-center text-[11px] mono transition ${
                            active ? "bg-sky-300 text-[#01040A]" : done ? "bg-sky-300/20 text-sky-200" : "bg-white/[0.04] text-slate-500 border border-white/10"
                          }`}>
                          {s.id}
                        </button>
                        <span className={`mono text-[10px] tracking-[0.18em] truncate ${active ? "text-sky-200" : "text-slate-500"}`}>{s.label}</span>
                        {s.id < STEPS.length && <span className="hidden sm:block flex-1 h-px bg-white/10" />}
                      </li>
                    );
                  })}
                </ol>
                  ...
                  >
                    {step === 1 && (
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <Field label="Company">
                          <input value={companyName} onChange={(e) => setCompanyName(e.target.value)} className="field" placeholder="Optional" />
                        </Field>
                        <Field label="Position">
                          <input value={position} onChange={(e) => setPosition(e.target.value)} className="field" placeholder="e.g. Marketing lead" />
                        </Field>
                        <Field label="Country">
                          <input value={country} onChange={(e) => setCountry(e.target.value)} className="field" placeholder="Optional" />
                        </Field>
                        <Field label="Phone">
                    ...
                    {step === 3 && (
                      <div className="space-y-5">
                        ...
                        <Field label="Or an exact amount" hint="Optional">
                          <input value={exactAmount} onChange={(e) => setExactAmount(e.target.value)} className="field" placeholder={`e.g. ${CURRENCY_META[currency].symbol} 12,000`} />
                        </Field>
                        ...
                        <Field label="Preferred contact" hint="Optional">
                          <div className="flex flex-wrap gap-2">
                            {CONTACT_METHODS.map((c) => (
                              <button type="button" key={c} onClick={() => setPreferredContact(c === preferredContact ? "" : c)}
                    ...
                    {step === 5 && (
                      <div className="space-y-5">
                        <Field label="Message" required error={errors.message}>
                          <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={5} className="field resize-none"
                            placeholder="Tell me about the brand, the challenge, the goals and any references…" />
                        </Field>
                        <Field label={`References (${files.length}/${MAX_FILES})`} hint="Up to 5 images, 8 MB each.">
                          {files.length > 0 && (
                            <div className="grid grid-cols-3 sm:grid-cols-5 gap-2 mb-3">
                              {files.map((f, i) => (
                                <div key={f.url} className="relative group rounded-lg overflow-hidden border border-white/10 bg-[#01040A] aspect-square">
...
function Field({ label, required, hint, error, className = "", children }: {
  label: string; required?: boolean; hint?: string; error?: string; className?: string; children: React.ReactNode;
}) {
  return (
