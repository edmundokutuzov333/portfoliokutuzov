...
function acceptableBody(request: Request) {
  ...
  server: {
    handlers: {
      ...
      GET: async ({ request }) => {
        ...
        return new Response(JSON.stringify({ status: "healthy", endpoint: "/api/studio/creative", providers: CREATIVE_PROVIDER_VALUES, limits: { maxRequests: MAX_REQUESTS, windowMinutes: 5, maxBodyBytes: MAX_BODY_BYTES }, requestId }), { status: 200, headers: headers(request, requestId, { "Content-Type": "application/json", "Cache-Control": "no-store" }) });
      },
      POST: async ({ request }) => {
        const requestId = getRequestId(request);
        ...
        if (!rate.allowed) return errorResponse(request, requestId, 429, "RATE_LIMITED", "Creative generation rate limit exceeded.", { "Retry-After": String(rate.retryAfter) });

        let body: CreativeEngineRequest;
        try { body = (await request.json()) as CreativeEngineRequest; } catch { return errorResponse(request, requestId, 400, "INVALID_JSON", "The request payload is not valid JSON."); }
        ...
        try {
          ...
          logObservability("request_end", { requestId, route: "/api/studio/creative", method: "POST", status: 200, latencyMs: Date.now() - startedAt, dependency: result.providers.map((item) => item.provider).join(",") });
