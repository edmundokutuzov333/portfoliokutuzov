...
export const Route = createFileRoute("/api/studio/creative")({
  server: {
    handlers: {
      ...
      GET: async ({ request }) => { if (!import.meta.env.VITE_STUDIO_PUBLIC_ENABLED || import.meta.env.VITE_STUDIO_PUBLIC_ENABLED !== "true") return studioUnavailableResponse(); const requestId = getRequestId(request); if (!isCorsOriginAllowed(request)) return errorResponse(request, requestId, 403, "ORIGIN_NOT_ALLOWED", "This origin is not allowed."); return new Response(JSON.stringify({ status: "healthy", endpoint: "/api/studio/creative", providers: CREATIVE_PROVIDER_VALUES, limits: { maxRequests: MAX_REQUESTS, windowMinutes: 5, maxBodyBytes: MAX_BODY_BYTES }, requestId }), { status: 200, headers: headers(request, requestId, { "Content-Type": "application/json", "Cache-Control": "no-store" }) }); },
      POST: async ({ request }) => {
