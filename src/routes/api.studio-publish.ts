...
export const Route = createFileRoute("/api/studio/publish")({
  server: { handlers: {
    POST: async ({ request }) => {
      ...
      void trackStudioEvent({ eventName: "card_published", sessionId: body.sessionId, cardId: (created as any)?.id ?? null, shareToken, durationMs: Date.now() - startedAt, metadata: { outcome: "success" } });
