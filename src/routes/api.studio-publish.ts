...
export const Route = createFileRoute("/api/studio/publish")({
  server: { handlers: {
    POST: async ({ request }) => {
      ...
      const row = {
        session_id: body.sessionId.trim().slice(0, 128),
      ...
      if (error) { logObservability("dependency_error", { requestId, route: "/api/studio/publish", dependency: "supabase", code: "STUDIO_PUBLISH_FAILED", message: error.message }); return response(request, requestId, 502, { error: "PUBLISH_FAILED" }); }
