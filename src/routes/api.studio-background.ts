...
function bodyTooLarge(request: Request) { const contentLength = request.headers.get("content-length"); if (!contentLength) return false; const size = Number(contentLength); return !Number.isFinite(size) || size < 0 || size > MAX_BODY_BYTES; }
  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        let body: { prompt?: unknown; negativePrompt?: unknown }; try { body = await request.json() as { prompt?: unknown; negativePrompt?: unknown }; } catch { return errorResponse(request, requestId, 400, "INVALID_JSON", "The request payload is not valid JSON."); }
        ...
        void trackStudioEvent({ eventName: "generation_started", provider: "magnific", metadata: { surface: "background" } });
        try {
          const result = await generateMagnificBackground(prompt, negativePrompt);
          void trackStudioEvent({ eventName: "generation_completed", provider: "magnific", durationMs: Date.now() - startedAt, metadata: { surface: "background" } });
          ...
          void trackStudioEvent({ eventName: "generation_failed", provider: "magnific", durationMs: Date.now() - startedAt, metadata: { surface: "background" } });
